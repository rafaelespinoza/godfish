...
// NewDriver creates a new MS SQL Server driver.
func NewDriver() godfish.Driver { return &driver{} }

// driver implements the godfish.Driver interface for MS SQL Server databases.
type driver struct {
	connection *sql.DB
}

func (d *driver) Name() string { return "mssqlserver" }
func (d *driver) Connect(dsn string) (err error) {
	if d.connection != nil {
		return
	}

	// github.com/denisenkom/go-mssqldb has two sql.Driver implementations:
