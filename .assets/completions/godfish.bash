...
__godfish_build_completion_request() {
...
__godfish_find_description_for() {
  local candidate="$1"
  local i
...
__godfish_bash_autocomplete() {
  ...
  if [[ "${words[0]}" != "source" ]]; then
    ...
    __godfish_init_completion -n "=:" || return

    request_comp="$(__godfish_build_completion_request)"
    ...
    if [[ "${COMP_TYPE:-}" == "63" && ${#matches[@]} -gt 0 ]]; then
      ...
      for candidate in "${matches[@]}"; do
        local desc="$(__godfish_find_description_for "${candidate}")"
...
complete -o bashdefault -o default -F __godfish_bash_autocomplete godfish
